...
public class HDBCalculationViewCreateProcessor extends HDBCalculationViewAbstractProcessor {
	...
	private static final Logger logger = LoggerFactory.getLogger(HDBCalculationViewCreateProcessor.class);

	private static HDBCalculationViewCreateProcessor INSTANCE = new HDBCalculationViewCreateProcessor();
	...
	private void deployContainerCreateContent(Connection connection, String container, List<XSKDataStructureHDBCalculationViewModel> hdbCalculationViews) throws SQLException {
    	...
    	for (XSKDataStructureHDBCalculationViewModel next : hdbCalculationViews) {
    		executeUpdate(connection, "INSERT INTO #DEPLOY_PATHS (PATH) VALUES ('" + next.getLocation().substring(1) + "');");
    	}
