...
import com.sap.xsk.hdb.ds.parser.hdbscalarfunction.XSKDataStructureHDBScalarFunctionModel;
...
public class XSKTopologyDataStructureModelWrapper implements ITopologicallySortable, ITopologicallyDepletable {
	...
	private final IXSKDataStructureManager<XSKDataStructureModel> xskProceduresManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_PROCEDURE);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskTableFunctionManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_TABLE_FUNCTION);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskScalarFunctionManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_SCALAR_FUNCTION);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskSynonymManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_SYNONYM);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskViewManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_VIEW);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskTableManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_TABLE);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskTableTypeManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_TABLE_TYPE);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskSchemaManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_SCHEMA);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskHdbddManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDBDD);
	private final IXSKDataStructureManager<XSKDataStructureModel> xskSequenceManagerService = managerServices.get(IXSKDataStructureModel.TYPE_HDB_SEQUENCE);
	...
	private boolean caseSensitive = Boolean.parseBoolean(Configuration.get(IDataStructureModel.DIRIGIBLE_DATABASE_NAMES_CASE_SENSITIVE, "false"));
	
	public XSKTopologyDataStructureModelWrapper(Connection connection, XSKDataStructureModel model, Map<String, XSKTopologyDataStructureModelWrapper> wrappers) {
	...
	public boolean complete(String flow) {
		try {
			...
			case EXECUTE_SCALAR_FUNCTION_DROP:
				if (model instanceof XSKDataStructureHDBScalarFunctionModel) {
					xskScalarFunctionManagerService.dropDataStructure(connection, this.model);
				}
				break;
				...
				if (model instanceof XSKDataStructureHDBTableModel) {
					...
					if (SqlFactory.getNative(connection).exists(connection, modelName)) {
						if (SqlFactory.getNative(connection).count(connection, modelName) == 0) {
							...
							logger.warn(message);
							applyArtefactState(this.model.getName(), this.model.getLocation(), TABLE_SYNCHRONIZATION_ARTEFACT_TYPE, ArtefactState.FAILED, message);
						}
					}
				}
				break;
			...
			case EXECUTE_TABLE_TYPE_CREATE:
				if (model instanceof XSKDataStructureHDBTableTypeModel) {
					if (SqlFactory.getNative(connection).exists(connection, model.getSchema(), model.getName(), DatabaseArtifactTypes.TABLE_TYPE)) {
						xskTableTypeManagerService.createDataStructure(connection, this.model);
					} else {
						String message = format("Table Type [{1}] in schema [{2}] already exists during the update process", this.model.getName(), this.model.getSchema());
						logger.warn(message);
						applyArtefactState(this.model.getName(), this.model.getLocation(), TABLE_TYPE_SYNCHRONIZATION_ARTEFACT_TYPE, ArtefactState.FAILED, message);
					}
				}
				break;
			case EXECUTE_TABLE_CREATE:
				if (model instanceof XSKDataStructureHDBTableModel) {
					...
					if (SqlFactory.getNative(connection).exists(connection, escapedName)) {
						...
						String message = format("Table [{1}] in schema [{2}] already exists during the update process", this.model.getName(), this.model.getSchema());
						logger.warn(message);
						if (SqlFactory.getNative(connection).count(connection, escapedName) != 0) {
							try {
								...
								applyArtefactState(this.model.getName(), this.model.getLocation(), TABLE_SYNCHRONIZATION_ARTEFACT_TYPE, ArtefactState.FAILED, e.getMessage());
							}
						} else {
							message = format("Table [{1}] in schema [{2}] already exists and it is not empty during the update process", this.model.getName(), this.model.getSchema());
							logger.warn(message);
							applyArtefactState(this.model.getName(), this.model.getLocation(), TABLE_SYNCHRONIZATION_ARTEFACT_TYPE, ArtefactState.FAILED, message);
						}
					}
			...
			case EXECUTE_VIEW_CREATE:
				if (model instanceof XSKDataStructureHDBViewModel) {
					if (SqlFactory.getNative(connection).exists(connection, model.getSchema(), model.getName(), DatabaseArtifactTypes.VIEW)) {
						...
						String message = format("View [{1}] in schema [{2}] already exists during the update process", this.model.getName(), this.model.getSchema());
						logger.warn(message);
						applyArtefactState(this.model.getName(), this.model.getLocation(), VIEW_SYNCHRONIZATION_ARTEFACT_TYPE, ArtefactState.FAILED, message);
					}
				}
				break;
			case EXECUTE_SYNONYM_CREATE:
				if (model instanceof XSKDataStructureHDBSynonymModel) {
					if (SqlFactory.getNative(connection).exists(connection, model.getSchema(), model.getName(), DatabaseArtifactTypes.SYNONYM)) {
						...
						String message = format("Synonym [{1}] in schema [{2}] already exists during the update process", this.model.getName(), this.model.getSchema());
						logger.warn(message);
						applyArtefactState(this.model.getName(), this.model.getLocation(), SYNONYM_SYNCHRONIZATION_ARTEFACT_TYPE, ArtefactState.FAILED, message);
					}
				}
				break;
			...
			case EXECUTE_SEQUENCE_CREATE:
				if (model instanceof XSKDataStructureHDBSequenceModel) {
					if (!SqlFactory.getNative(connection).exists(connection, model.getSchema(), this.model.getName(), DatabaseArtifactTypes.SEQUENCE)) {
						...
						try {
							...
							logger.error(e.getMessage(), e);
							applyArtefactState(this.model.getName(), this.model.getLocation(), SEQUENCE_SYNCHRONIZATION_ARTEFACT_TYPE, ArtefactState.FAILED, e.getMessage());
						}
					}
				}
				break;
			...
			case EXECUTE_SCALAR_FUNCTION_CREATE:
				if (model instanceof XSKDataStructureHDBScalarFunctionModel) {
					xskScalarFunctionManagerService.createDataStructure(connection, this.model);
				}
				break;
