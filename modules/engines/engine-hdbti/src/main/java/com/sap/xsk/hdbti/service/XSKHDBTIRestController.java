...
public class XSKHDBTIRestController extends AbstractRestService {

    private static final Logger logger = LoggerFactory.getLogger(XSKHDBTIRestController.class);

    @Context
    private HttpServletResponse response;

    private final XSKHDBTIRestService hdbtiRestService = new XSKHDBTIRestService();
    ...
    public Response parseHdbtiToJSON(@ApiParam(value = "File registry path") @QueryParam("location") String location,
        ...
        try {
            return Response.ok(hdbtiRestService.parseHdbtiToJSON(location, file)).build();
        } catch (Throwable e) {
    ...
    public Response parseJSONtoHdbti(ArrayList<XSKHDBTIImportConfigModel> json) {
        try {
            return Response.ok(hdbtiRestService.parseJSONtoHdbti(json)).build();
        } catch (Throwable e) {
            String message = e.getMessage();
            logger.error(message, e);
            createErrorResponseInternalServerError(message);
    ...
    public Class<? extends IRestService> getType() {
        return XSKHDBTIRestController.class;
    }
