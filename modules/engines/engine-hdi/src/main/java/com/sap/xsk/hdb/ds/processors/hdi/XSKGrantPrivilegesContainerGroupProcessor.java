  ...
  public final void execute(Connection connection, String group, String[] users) throws SQLException {
    ...
    for (String user : users) {
      executeUpdate(connection, "INSERT INTO #PRIVILEGES (PRINCIPAL_NAME, PRIVILEGE_NAME, OBJECT_NAME) SELECT '" + user
          + "', PRIVILEGE_NAME, OBJECT_NAME FROM _SYS_DI.T_DEFAULT_CONTAINER_GROUP_ADMIN_PRIVILEGES;");
    }
    executeQuery(connection,
        "CALL _SYS_DI.GRANT_CONTAINER_GROUP_API_PRIVILEGES('" + group + "', #PRIVILEGES, _SYS_DI.T_NO_PARAMETERS, ?, ?, ?);");
