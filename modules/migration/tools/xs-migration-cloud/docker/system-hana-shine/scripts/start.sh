...
function __xsa_create_and_start_container
{
    ...
    is_empty "$SYSTEM_TYPE_PREFIX" \
        &&__xsa_logging_log_fatal "no system type prefix provided"
    ...
    local PORTS="$4"
    is_empty "$PORTS" \
        && __xsa_logging_log_fatal "no ports to exposed provided"
    ...
    IMAGE_NAME="xsa/system-hana-shine"
